...
この文書は、`docs/improve-dictation.md` のセカンドオピニオンを受けて、実際に何をどういう順番で改善したかを残すための実装記録。
...
docs/local-dictation-improvement-log.md
```
...
`docs/improve-dictation.md` では、次の方針が提案された。
...
`docs/improve-dictation.md` の提案を受けて、`transcribe.sh` で次の引数を渡すようにした。
