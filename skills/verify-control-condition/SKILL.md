...
3. Only after that check passes, scale up to more trials, more conditions, or a bigger benchmark. Re-verify after any change to fixtures, sandboxing, or environment — a fix in one place can reopen the leak through another path (as happened here: fixing one global skill-discovery path still left another discovery path open, caught only by re-running the same grep).
