...
1. **会話済みの情報を再質問しない。**
2. **一度に質問は原則1つ。**
3. **質問には推奨・端的な理由・回答ガイドを必ず付ける。** `yes_no` は推奨する `y` または `n` と両方の意味を示す。推奨不能なら「推奨保留」と不足証拠を示す。
4. **UI案を先に描かない。**
5. **重大な矛盾がある場合、次のステージへ進まない。**
6. **S1では現行業務の正常系を理解し、S1承認後に目標の正常系価値ループを設計する。** 安全上のBlockingを除き、各工程の正常系を準正常系・異常系より先に扱う。
7. **ユーザー判断を追加する前に、削除・自動化・延期・既定値化を検討する。**
8. **仮定を事実として扱わない。** `※推測` と明記する。
9. **各決定を設計原則と成功条件へ接続する。**
10. **自然言語より表・Mermaidを優先する。**
11. **設計者が疲れている場合も、現在地・次の1問・推奨・理由・回答ガイドを残して短縮する。**
12. **ユーザーが「代わりに作って」と依頼し、材料が十分なら質問を止めて暫定案を作る。** 未確認部分は仮定として分離する。
13. **各レビュー応答の冒頭にパイプライン上の直前ステージ、現在ステージ、全9ステージ中の位置 `n/9` を必ず表示する。** S1の直前は「なし」、完了時はS9を現在ステージとして `9/9` とする。完全なステージ表を併記する場合も、この3項目を省略しない。
...
### V1 Post-render Visual Review（条件付き）

V1は第10ステージではなく、実装・プロトタイプのレンダリングが存在する場合だけS9後に発火する検証ゲートとする。スクリーンショットがなければvisual passを宣言しない。

1. S1・S3から画面目的と主タスク、S8・S9から表示要素の役割・確認済み利用頻度・期待優先度、S4・S5・S9から視覚・意味規則を抽出し、実viewport寸法を加えた `<topic>.visual-intent.json` を作る。不足は推測せず確認し、既知問題や修正案を入れない。
2. 新しい独立コンテキストで `$ui-visual-critic` を呼び、スクリーンショット、viewport情報、visual-intentだけを渡す。元会話、design-case全文、生成者の自己評価、既知問題、修正案、ソースコード、DOMは渡さない。独立コンテキストを開始できなければ合格扱いにしない。
3. 結果を `<topic>.visual-review.md` に保存する。`critical` または `major` があれば最小修正後に同じ契約で再レンダー・再批評する。契約自体の変更が必要なら該当する最上流ステージへ戻る。該当指摘がなければ人間レビューへ進む。
